...
def test_evidence_bundle_includes_litigation_metadata():
    ...
    certificate = {
        ...
        "integrity_chain": {"chain_hash": "chain123", "previous_hash": None},
    }
...
def test_litigation_metadata_human_attestation_summary():
    ...
    summary_reviewed = bundle_reviewed["litigation_metadata"][
    ...
    summary_not_reviewed = bundle_not_reviewed["litigation_metadata"][
        "human_attestation_summary"
    ]
...
def test_defense_bundle_nonexistent_certificate_returns_404(client):
    ...
    headers = create_clinician_headers("hospital-alpha")
    response = client.get(
        "/v1/certificates/NONEXISTENT-CERT/defense-bundle", headers=headers
    )
...
def test_generate_defense_bundle_function():
    ...
    certificate = {
        ...
        "signature": {
            "key_id": "key-001",
            "canonical_message": {"certificate_id": "TEST-001", "note_hash": "hash123"},
        },
    }
...
-----END PUBLIC KEY-----"""
    ...
    verification_report = {"status": "VALID", "verified_at": "2024-01-15T11:00:00Z"}

    # Generate bundle
    zip_bytes = generate_defense_bundle(
        certificate, public_key_pem, verification_report
    )
...
def test_defense_bundle_verification_report_included(client):
    ...
    with zipfile.ZipFile(zip_buffer, "r") as zf:
        ...
        assert (
            "checks" in verify_report
            or "summary" in verify_report
