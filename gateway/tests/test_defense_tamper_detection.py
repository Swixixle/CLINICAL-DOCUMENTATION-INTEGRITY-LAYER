...
def test_simulate_alteration_detects_tampering(client):
    ...
    response = client.post("/v1/defense/simulate-alteration", json=altered_request, headers=headers)
    ...
    assert "Tampering detected" in data["summary"] or "tampering detected" in data["summary"]
...
def test_simulate_alteration_single_character_change_detected(client):
    ...
    altered_request = {
        "certificate_id": cert_id,
        "modified_note_text": modified_note
    }
