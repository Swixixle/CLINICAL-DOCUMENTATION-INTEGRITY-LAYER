...
def test_chf_missing_treat_triggers_risk(client):
    ...
    chf_treat_exp = [exp for exp in explanations if exp["rule_id"] == "CHF_TREAT_MISSING"][0]
    assert chf_treat_exp["impact"] == 25
    
    # Check that deficits include CHF treat deficit
    deficits = data["deficits"]
    chf_treat_deficits = [d for d in deficits if d.get("condition") == "chf" and d.get("category") == "treat"]
    ...
    assert "furosemide" in chf_deficit["fix"].lower() or "medication" in chf_deficit["fix"].lower()
...
def test_revenue_estimate_outpatient_high_risk(client):
    ...
    response_outpatient = client.post("/v1/shadow/evidence-deficit", json=failing_note, headers=headers)
    ...
    response_inpatient = client.post("/v1/shadow/evidence-deficit", json=failing_note, headers=headers)
...
def test_exec_headline_generation(client):
    ...
    headers = create_clinician_headers("test-tenant-001")
    response = client.post("/v1/shadow/evidence-deficit", json=low_risk_note, headers=headers)
    assert response.status_code == 200
    data = response.json()
