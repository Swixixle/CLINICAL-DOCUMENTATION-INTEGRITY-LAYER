...
def test_proof_1_tenant_spoof_rejected(client):
    ...
    tenant_a_token = generate_test_jwt(
        sub="attacker-001", tenant_id="hospital-a", role="clinician"
    )
    ...
    assert cert["signature"]["key_id"].startswith(
        "key-"
...
def test_proof_2_cross_tenant_forge_impossible(client):
    ...
    tenant_a_token = generate_test_jwt(
        sub="user-a", tenant_id="hospital-a", role="clinician"
    )
    ...
    tenant_b_token = generate_test_jwt(
        sub="user-b", tenant_id="hospital-b", role="clinician"
    )
    ...
    assert (
        cert_a["signature"]["signature"] != cert_b["signature"]["signature"]
...
def test_proof_3_cross_tenant_read_blocked(client):
    ...
    tenant_a_token_auditor = generate_test_jwt(
        sub="auditor-a", tenant_id="hospital-a", role="auditor"
    )
    ...
    tenant_b_token = generate_test_jwt(
        sub="user-b", tenant_id="hospital-b", role="auditor"
    )
...
def test_proof_4_key_rotation_preserves_old_certs(client):
    ...
    tenant_token = generate_test_jwt(
        sub="user-001", tenant_id="hospital-rotation-test", role="clinician"
    )
    ...
    auditor_token = generate_test_jwt(
        sub="auditor-001", tenant_id="hospital-rotation-test", role="auditor"
    )
    ...
    assert (
        verify_old_after.json()["valid"] is True
...
def test_proof_5_audit_pack_completeness(client):
    ...
    # Create certificate
    tenant_token = generate_test_jwt(
        sub="user-001", tenant_id="hospital-audit", role="clinician"
    )
    ...
    auditor_token = generate_test_jwt(
        sub="auditor-001", tenant_id="hospital-audit", role="auditor"
    )
...
def test_insufficient_role_rejected(client):
    ...
    clinician_token = generate_test_jwt(
        sub="clinician-001", tenant_id="hospital-rbac", role="clinician"
    )
    ...
    # Auditor can verify
    auditor_token = generate_test_jwt(
        sub="auditor-001", tenant_id="hospital-rbac", role="auditor"
    )
    ...
    get_response_auditor = client.get(
        f"/v1/certificates/{cert_id}", headers=create_auth_headers(token=auditor_token)
    )
...
def test_expired_token_rejected(client):
    ...
    expired_token = generate_expired_jwt(
        sub="user-001", tenant_id="hospital-x", role="clinician"
    )
    ...
    if "detail" in json_response:
        ...
        assert (
            "invalid_token" in error.get("error", "")
            or "expired" in error.get("message", "").lower()
        )
    else:
        # Error might be at root level
        assert (
            "detail" in str(json_response).lower()
            or "expired" in str(json_response).lower()
        )
