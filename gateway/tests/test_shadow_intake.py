...
class TestShadowIntake:
    ...
    def test_shadow_intake_no_phi_leakage_by_default(self):
        ...
        response = client.post(
            "/v1/shadow/intake",
            headers=headers,
            json={"note_text": note_text, "patient_reference": "PATIENT-456"},
        )
        ...
        get_response = client.get(f"/v1/shadow/items/{shadow_id}", headers=headers)
    ...
    def test_shadow_intake_tenant_isolation(self):
        ...
        response = client.post(
            "/v1/shadow/intake",
            headers=headers_t1,
            json={"note_text": "Tenant 1 note text"},
        )
        ...
        get_response = client.get(f"/v1/shadow/items/{shadow_id}", headers=headers_t2)
    ...
    def test_shadow_intake_validates_minimum_note_length(self):
        ...
        response = client.post(
            "/v1/shadow/intake",
            headers=headers,
            json={"note_text": "Short"},  # Less than 10 characters
        )
    ...
    def test_shadow_intake_list_items(self):
        ...
        for i in range(3):
            client.post(
                "/v1/shadow/intake",
                headers=headers,
                json={"note_text": f"Test note {i} with sufficient length"},
            )

        # List items
        response = client.get(
            "/v1/shadow/items", headers=headers, params={"page": 1, "page_size": 10}
        )
    ...
    def test_shadow_intake_requires_authentication(self):
        ...
        response = client.post(
            "/v1/shadow/intake", json={"note_text": "Test note without authentication"}
        )
