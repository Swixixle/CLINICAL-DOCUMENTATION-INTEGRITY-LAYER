...
def estimate_preventable_revenue_loss(
    ...
    if has_high_scrutiny:
        assumptions.append(
            f"High-scrutiny diagnosis multiplier: {params['high_scrutiny_multiplier']}x"
        )
...
def generate_next_best_actions(
    deficits: List[EvidenceDeficit], max_actions: int = 3
) -> List[str]:
    """
...
def build_dashboard_payload(
    ...
    revenue_estimate = estimate_preventable_revenue_loss(
        request.encounter_type, deficits, risk_flags
    )
    ...
    return ShadowResult(
        ...
        evidence_sufficiency=EvidenceSufficiency(
            score=score, band=score_band, explain=explanations
        ),
        deficits=deficits,
        denial_risk=DenialRisk(
            flags=risk_flags, estimated_preventable_revenue_loss=revenue_estimate
        ),
        audit=AuditMetadata(ruleset_version=ruleset_version, inputs_redacted=True),
