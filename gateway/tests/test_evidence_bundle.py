...
def test_evidence_bundle_json_structure(client):
    ...
    response = client.get(
        f"/v1/certificates/{certificate_id}/evidence-bundle.json", headers=headers
    )
...
def test_evidence_bundle_cross_tenant_access_forbidden(client):
    ...
    response = client.get(
        f"/v1/certificates/{certificate_id}/evidence-bundle.json",
...
def test_evidence_bundle_no_phi_in_response(client):
    ...
    assert (
        "Patient presents with headache" not in bundle_str
    )  # note_text should be hashed
    assert "DR-JANE-DOE" not in bundle_str  # reviewer_id should be hashed
    assert (
        "PATIENT-JOHN-DOE-12345" not in bundle_str
    )  # patient_reference should be hashed
...
def test_evidence_bundle_zip_includes_json_bundle(client):
    ...
    response = client.get(
        f"/v1/certificates/{certificate_id}/evidence-bundle.zip", headers=headers
    )
...
def test_evidence_bundle_not_found(client):
    ...
    response = client.get(
        "/v1/certificates/fake-cert-id-999/evidence-bundle.json", headers=headers
    )
...
def test_evidence_bundle_includes_chain_integrity(client):
    ...
    issue_test_certificate(client, tenant_id="tenant-chain-001")
