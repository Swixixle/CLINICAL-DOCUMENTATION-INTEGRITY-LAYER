...
class ClinicalDocumentationRequest(BaseModel):
    ...
    model_name: str = Field(
        ..., description="AI model name/identifier (e.g., 'gpt-4', 'claude-3')"
    )
    model_version: str = Field(
        ..., description="AI model version used to generate note"
    )
    ...
    human_reviewed: bool = Field(
        ..., description="Whether note was reviewed by clinician"
    )

    # Optional fields
    human_reviewer_id: Optional[str] = Field(
        default=None, description="Reviewer ID (will be hashed if provided)"
    )
    encounter_id: Optional[str] = Field(
        default=None, description="Encounter/visit identifier"
    )
    patient_reference: Optional[str] = Field(
        default=None, description="Patient reference (will be hashed if provided)"
    )
...
class IntegrityChain(BaseModel):
    ...
    previous_hash: Optional[str] = Field(
        default=None, description="Hash of previous certificate in chain"
    )
    chain_hash: str = Field(
        ..., description="Hash of this certificate including chain linkage"
    )
...
class DocumentationIntegrityCertificate(BaseModel):
    ...
    timestamp: str = Field(
        ..., description="Certificate issuance timestamp (ISO 8601 UTC)"
    )
    issued_at_utc: str = Field(
        ..., description="When certificate was issued (ISO 8601 UTC) - signed field"
    )

    # Timing integrity
    finalized_at: str = Field(
        ..., description="When note was finalized and certificate issued (ISO 8601 UTC)"
    )
    ehr_referenced_at: Optional[str] = Field(
        default=None,
        description="When EHR/record references the note/cert (ISO 8601 UTC)",
    )
    ehr_commit_id: Optional[str] = Field(
        default=None, description="Opaque EHR reference string (no PHI)"
    )
    ...
    # Governance metadata
    governance_policy_version: str = Field(..., description="Governance policy version")
    governance_policy_hash: str = Field(
        ..., description="Hash of governance policy document (signed)"
    )

    # Governance provenance (legacy field)
    policy_hash: str = Field(
        ..., description="Hash of governance policy document (legacy)"
    )
    ...
    patient_hash: Optional[str] = Field(
        default=None, description="SHA-256 hash of patient reference"
    )
    reviewer_hash: Optional[str] = Field(
        default=None, description="SHA-256 hash of reviewer ID"
    )
    ...
    human_reviewer_id_hash: Optional[str] = Field(
        default=None, description="SHA-256 hash of reviewer ID (signed field)"
    )
    human_attested_at_utc: Optional[str] = Field(
        default=None,
        description="When human attestation occurred (ISO 8601 UTC) - signed field",
    )
    ...
    # Integrity chain
    integrity_chain: IntegrityChain = Field(
        ..., description="Chain linkage for tamper detection"
    )
...
class CertificateIssuanceResponse(BaseModel):
    ...
    certificate_id: str = Field(..., description="Unique certificate identifier")
    certificate: DocumentationIntegrityCertificate = Field(
...
class ClinicalDocRequest(BaseModel):
    ...
    ai_vendor: str = Field(
        ..., description="AI vendor/provider name (e.g., 'openai', 'anthropic')"
    )
    model_version: str = Field(
        ..., description="AI model version (e.g., 'gpt-4-turbo')"
    )
    ...
    # Review information
    human_reviewed: bool = Field(
        default=False, description="Whether a human reviewed the note"
    )
    human_editor_id: Optional[str] = Field(
        default=None, description="Identifier of human reviewer/editor"
    )

    # Optional metadata
    note_type: Optional[str] = Field(
        default=None,
        description="Type of clinical note (e.g., 'progress_note', 'discharge_summary')",
    )
...
class ClinicalDocumentationCertificate(BaseModel):
    ...
    governance_checks: list[str] = Field(
        default_factory=list, description="List of governance checks executed"
    )
...
class ClinicalDocResponse(BaseModel):
    ...
    hash_prefix: str = Field(
        ..., description="First 8 characters of final hash for quick reference"
    )
    certificate: ClinicalDocumentationCertificate = Field(
