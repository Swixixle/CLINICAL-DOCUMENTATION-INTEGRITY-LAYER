...
def test_timing_integrity_backdating_detected(client):
    ...
    try:
        ...
        finalized_at = datetime.fromisoformat(cert['finalized_at'].replace('Z', '+00:00'))
        ehr_referenced_at = (finalized_at - timedelta(hours=1)).isoformat().replace('+00:00', 'Z')
    ...
    assert "backdating" in report["summary"].lower() or "timing" in report["summary"].lower()
    assert "backdated" in report["reason"].lower() or "temporal" in report["reason"].lower()
