...
def test_cross_tenant_verify_isolation(client):
    ...
    assert verify_response_a.json()["valid"]
...
def test_missing_tenant_header_rejected(client):
    ...
    response = client.post(
        "/v1/clinical/documentation",
        json=request_data,
        headers=create_clinician_headers("test-tenant"),
    )
...
def test_note_text_never_persisted(client, test_db):
    ...
    """
    sensitive_note_text = (
        "Extremely sensitive clinical information about patient condition."
    )
    ...
    for row in rows:
        cert_json = row[0]
        assert (
            sensitive_note_text not in cert_json
        ), "Plaintext note_text found in database!"
        assert (
            "MRN-SENSITIVE-12345" not in cert_json
        ), "Plaintext patient_reference found in database!"
        assert (
            "DR-SENSITIVE-ID" not in cert_json
        ), "Plaintext reviewer_id found in database!"

        # Verify hashes ARE present
        cert_dict = json.loads(cert_json)
        assert "note_hash" in cert_dict, "note_hash missing"
        assert (
            cert_dict["note_hash"].startswith("sha256:")
            or len(cert_dict["note_hash"]) == 64
        ), "Invalid note_hash format"
...
def test_chain_integrity_per_tenant(client):
    ...
    response_a1 = client.post(
        "/v1/clinical/documentation",
        json=request_a1,
        headers=create_clinician_headers("tenant-chain-alpha"),
    )
    ...
    response_b1 = client.post(
        "/v1/clinical/documentation",
        json=request_b1,
        headers=create_clinician_headers("tenant-chain-beta"),
    )
    ...
    assert (
        cert_a2["integrity_chain"]["previous_hash"]
        == cert_a1["integrity_chain"]["chain_hash"]
    )

    # Verify it does NOT link to tenant B's chain
    assert (
        cert_a2["integrity_chain"]["previous_hash"]
        != cert_b1["integrity_chain"]["chain_hash"]
    )
...
def test_query_certificates_tenant_isolation(client):
    ...
    response_a = client.post(
        "/v1/clinical/documentation",
        json=request_a,
        headers=create_clinician_headers("tenant-query-alpha"),
    )
    assert response_a.status_code == 200
    cert_a_id = response_a.json()["certificate_id"]
    ...
    response_b = client.post(
        "/v1/clinical/documentation",
        json=request_b,
        headers=create_clinician_headers("tenant-query-beta"),
    )
    ...
    query_a = client.post(
        "/v1/certificates/query", headers=create_auditor_headers("tenant-query-alpha")
    )
    ...
    assert all(
        c["tenant_id"] == "tenant-query-alpha" for c in results_a["certificates"]
    )

    # Query as tenant B (auditor role required)
    query_b = client.post(
        "/v1/certificates/query", headers=create_auditor_headers("tenant-query-beta")
    )
