...
class ModelRequest(BaseModel):
    ...
    provider: str = Field(
        default="openai", description="AI provider (e.g., 'openai', 'anthropic')"
    )
...
class AICallRequest(BaseModel):
    ...
    environment: Literal["prod", "staging", "dev"] = Field(
        ..., description="Environment: prod, staging, or dev (REQUIRED)"
    )
    client_id: str = Field(..., description="Client identifier (REQUIRED)")
    feature_tag: str = Field(
        ..., description="Feature tag (e.g., billing, customer-support)"
    )
    ...
    prompt: Union[str, List[Any]] = Field(
        ..., description="The prompt text or messages to send to the AI model"
    )

    # Optional context
    rag_context: Optional[Union[Dict[str, Any], List[Any]]] = Field(
        default=None, description="Optional RAG context"
    )
    ...
    model_request: Optional[ModelRequest] = Field(
        default=None, description="Model configuration (new format)"
    )
    model: Optional[str] = Field(default=None, description="Model identifier (legacy)")
    temperature: Optional[float] = Field(
        default=None, description="Model temperature (legacy)"
    )
