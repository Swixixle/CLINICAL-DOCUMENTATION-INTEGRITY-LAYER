...
def _categorize_failures(failures: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    for failure in failures:
        ...
        elif "timing" in check.lower() or "finalized" in error.lower() or "backdated" in error.lower():
...
def _generate_summary(categories: Dict[str, List[str]]) -> str:
    ...
    if categories["integrity"]:
        return "Certificate verification FAILED: Document has been altered since issuance."
...
def _generate_reason(categories: Dict[str, List[str]], failures: List[Dict[str, Any]]) -> str:
    """Generate detailed reason for failure."""
    reasons = []
    
    if categories["integrity"]:
        reasons.append("The document content or certificate metadata has been modified after the certificate was issued. " +
                      "This breaks the cryptographic integrity chain and indicates tampering.")
    
    if categories["signature"]:
        reasons.append("The cryptographic signature does not match the certificate contents. " +
                      "This could indicate forgery or corruption during transmission.")
    
    if categories["timing"]:
        reasons.append("The certificate finalization timestamp is after the EHR reference timestamp. " +
                      "This suggests the certificate may have been backdated, which violates temporal integrity requirements.")
    
    if categories["policy"]:
        reasons.append("The governance policy hash does not match or policy information is missing. " +
                      "This prevents verification of which rules and compliance checks were applied.")
    
    if categories["tenant"]:
        reasons.append("The certificate tenant identifier does not match the requesting organization. " +
                      "Access to this certificate is restricted to the issuing tenant.")
    
    if categories["other"]:
        reasons.append("Additional integrity checks failed. Review detailed error list.")
...
def _generate_recommended_action(categories: Dict[str, List[str]]) -> str:
    ...
    if categories["tenant"]:
        return ("Contact the certificate issuing organization to verify access permissions. " +
               "Do not attempt to use this certificate for compliance or legal purposes.")
    
    if categories["timing"]:
        return ("Investigate the certificate issuance timeline. Contact the issuing organization to verify " +
    ...
    if categories["integrity"] or categories["signature"]:
        return ("DO NOT USE this certificate. The document has failed cryptographic verification. " +
    ...
    if categories["policy"]:
        return ("Contact the certificate issuing organization to obtain the correct governance policy documentation. " +
               "Without policy verification, compliance claims cannot be validated.")
    
    return ("Review the detailed failure information below. Contact the certificate issuing organization " +
           "for assistance. Do not rely on this certificate until all integrity checks pass.")


def _interpret_failure(check: str, error: str, debug: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
