...
async def get_executive_summary(
    ...
    from_date: Optional[str] = Query(None, alias="from", description="Start date (ISO 8601 UTC)"),
    to_date: Optional[str] = Query(None, alias="to", description="End date (ISO 8601 UTC)")
) -> Dict[str, Any]:
    """
...
async def get_risk_queue(
    identity: Identity = Depends(get_current_identity),
    band: Optional[str] = Query(None, description="Risk band filter (HIGH, MEDIUM, LOW)"),
    ...
    for item in result["items"]:
        ...
        queue_items.append({
            ...
            "export_links": {
                "evidence_bundle_json": f"/v1/certificates/{item.get('certificate_id')}/evidence-bundle.json" if item.get("certificate_id") else None,
                "evidence_bundle_zip": f"/v1/certificates/{item.get('certificate_id')}/evidence-bundle.zip" if item.get("certificate_id") else None
            }
        })
