...
class ShadowRequest(BaseModel):
    ...
    note_text: str = Field(
        ..., description="Clinical note text (will be hashed, never stored)"
    )
    encounter_type: EncounterType = Field(
        ..., description="Type: inpatient|observation|outpatient|ed"
    )
    service_line: ServiceLine = Field(
        ..., description="Service: medicine|surgery|icu|cardiology|other"
    )
    diagnoses: List[str] = Field(
        default_factory=list, description="List of diagnosis codes or descriptions"
    )
    procedures: List[str] = Field(
        default_factory=list, description="List of procedure codes or descriptions"
    )
    ...
    problem_list: List[str] = Field(
        default_factory=list, description="Active problem list"
    )
    meds: List[str] = Field(default_factory=list, description="Current medications")
    discharge_disposition: Optional[str] = Field(
        default=None, description="Discharge disposition if applicable"
    )
...
class ScoreExplanation(BaseModel):
    ...
    impact: int = Field(
        ..., description="Score impact (negative = penalty, positive = bonus)"
    )
...
class EvidenceSufficiency(BaseModel):
    ...
    score: int = Field(
        ..., ge=0, le=100, description="Evidence sufficiency score (0-100)"
    )
    band: str = Field(..., description="Risk band: low|moderate|high|critical")
    explain: List[ScoreExplanation] = Field(
        ..., description="List of scoring rule applications"
    )
...
class EvidenceReference(BaseModel):
    ...
    type: str = Field(
        ..., description="Evidence type: lab|vital|diagnosis|problem|med|note_text"
    )
...
class EvidenceDeficit(BaseModel):
    ...
    category: str = Field(
        ...,
        description="Category: documentation|coding|clinical_inconsistency|monitor|evaluate|assess|treat",
    )
    why_payer_denies: str = Field(
        ..., description="Payer perspective on why this is vulnerable"
    )
    what_to_add: str = Field(
        ..., description="Provider-facing guidance on what to document"
    )
    evidence_refs: List[EvidenceReference] = Field(
        default_factory=list, description="Referenced evidence"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in this finding (0-1)"
    )
    # MEAT-specific fields (optional for non-MEAT deficits)
    condition: Optional[str] = Field(
        default=None,
        description="Condition name for MEAT deficits (diabetes|hypertension|chf)",
    )
    missing: Optional[List[str]] = Field(
        default=None, description="List of specific missing anchors for MEAT deficits"
    )
    fix: Optional[str] = Field(
        default=None,
        description="Explicit one-sentence fix instruction for MEAT deficits",
    )
...
class DenialRisk(BaseModel):
    ...
    score: int = Field(
        ..., ge=0, le=100, description="Denial risk score (0-100, higher = higher risk)"
    )
    band: RiskBand = Field(..., description="Risk band: low|moderate|high|critical")
    primary_reasons: List[str] = Field(
        ..., description="Top 3 reasons for denial risk (by impact)"
    )
    flags: List[DenialRiskFlag] = Field(
        default_factory=list, description="List of risk flags"
    )
    estimated_preventable_revenue_loss: RevenueEstimate = Field(
...
class AuditMetadata(BaseModel):
    ...
    inputs_redacted: bool = Field(
        default=True, description="Whether inputs were redacted from storage"
    )
...
class ShadowResult(BaseModel):
    ...
    evidence_sufficiency: EvidenceSufficiency = Field(
        ..., description="Evidence sufficiency scoring"
    )
    deficits: List[EvidenceDeficit] = Field(
        ..., description="Identified documentation deficits"
    )
    ...
    dashboard_title: str = Field(
        default="Evidence Deficit Intelligence", description="Dashboard title"
    )
    ...
    revenue_estimate: float = Field(
        default=0.0, ge=0, description="Estimated revenue impact (USD)"
    )
