...
def test_hash_prefixes_are_limited_to_16_chars(client):
    ...
    assert (
        len(original_final_hash) > 16
    ), f"Hash should be longer than prefix (16 chars), got {len(original_final_hash)}"
    # Typical format: "sha256:64hexchars" = 71 chars total
    assert original_final_hash.startswith(
        "sha256:"
    ), f"Expected sha256: prefix, got {original_final_hash[:10]}..."
    # Verify the hash portion (after algorithm prefix) is at least 64 chars
    hash_portion = (
        original_final_hash.split(":", 1)[1]
        if ":" in original_final_hash
        else original_final_hash
    )
    assert (
        len(hash_portion) >= 64
    ), f"Expected at least 64 hex chars in hash, got {len(hash_portion)}"
...
def test_verify_utils_fail_helper():
    ...
    f2 = fail(
        "halo_chain",
        "final_hash_mismatch",
        {"stored_prefix": "abc", "recomputed_prefix": "def"},
    )
    assert f2 == {
        "check": "halo_chain",
