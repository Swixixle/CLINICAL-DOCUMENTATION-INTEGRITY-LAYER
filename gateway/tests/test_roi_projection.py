...
def test_roi_projection_happy_path_conservative(client, conservative_inputs):
    ...
    assert (
        data["assumptions"]["annual_revenue"] == conservative_inputs["annual_revenue"]
    )
    assert (
        data["assumptions"]["denial_prevention_rate"]
        == conservative_inputs["denial_prevention_rate"]
    )
...
def test_roi_projection_rejects_documentation_denial_ratio_above_one(
    client, conservative_inputs
):
...
def test_roi_projection_zero_denial_rate(client, conservative_inputs):
    ...
    response = client.post(
        "/v2/analytics/roi-projection", json=inputs_with_zero_denials
    )
...
def test_roi_projection_maximum_prevention_rate(client, conservative_inputs):
    ...
    response = client.post(
        "/v2/analytics/roi-projection", json=inputs_with_max_prevention
    )
    ...
    assert (
        abs(data["prevented_denials_revenue"] - data["documentation_denied_revenue"])
        < 1.0
    )
...
def test_calculate_roi_assumptions_echo():
    ...
    assert (
        outputs.assumptions.documentation_denial_ratio
