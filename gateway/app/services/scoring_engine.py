...
# Diabetes anchors
DIABETES_KEYWORDS = [
    "diabetes", "dm2", "t2dm", "diabetic", "e11.9", "e10", "e11"
]
DIABETES_MONITOR = [
    "glucose", "blood sugar", "fingerstick", "fsbs", "a1c", "hba1c", "cgm"
]
DIABETES_EVALUATE = [
    "controlled", "uncontrolled", "at goal", "above goal", "improving", "worsening"
]
DIABETES_ASSESS = [
    "diabetes", "dm", "controlled", "uncontrolled", "a1c"
]
DIABETES_TREAT = [
...
HTN_KEYWORDS = [
    "hypertension", "htn", "i10", "elevated bp", "high blood pressure"
]
HTN_MONITOR = [
    "bp", "blood pressure", "home bp", "ambulatory", "log"
]
HTN_EVALUATE = [
    "controlled", "uncontrolled", "at goal", "above goal", "improving", "worsening"
]
...
# CHF anchors
CHF_KEYWORDS = [
    "heart failure", "chf", "hfref", "hfpef", "i50", "congestive heart failure"
]
CHF_MONITOR = [
    "weight", "daily weight", "edema", "swelling", "i/o", "oxygen", "sob", "dyspnea"
]
CHF_EVALUATE = [
    "euvolemic", "volume overloaded", "improving", "worsening", "stable", "exacerbation"
]
CHF_ASSESS = [
    "hfref", "hfpef", "ef", "ejection fraction", "nyha"
]
CHF_TREAT = [
...
VAGUE_PLAN_PHRASES = [
    "follow up", "f/u", "continue current", "monitor", "stable", "as above", "no changes"
]
...
def check_vague_plan(note_text: str) -> bool:
    ...
    for phrase in VAGUE_PLAN_PHRASES:
        ...
        if match:
            ...
            action_words = ["start", "stop", "increase", "decrease", "add", "discontinue", "adjust", "change"]
    ...
    def score(
        ...
        if len(note_text) < 400:
            ...
            deficits.append(EvidenceDeficit(
                ...
                evidence_refs=[EvidenceReference(type="note_text", key="length", value=len(note_text))],
                confidence=0.95
            ))
        
        # Rule 2: Check for diagnoses
        if not diagnoses and not contains_any_keyword(note_text, DIABETES_KEYWORDS + HTN_KEYWORDS + CHF_KEYWORDS):
            risk_score += 20
            explanations.append(ScoreExplanation(
