...
class RoiInputs(BaseModel):
    ...
    annual_revenue: float = Field(
        ..., description="Annual Net Patient Service Revenue (NPSR) in USD", gt=0
    )
    denial_rate: float = Field(
        ..., description="Overall denial rate (0.00 to 1.00)", ge=0, le=1
    )
    ...
    cost_per_appeal: float = Field(
        ..., description="Average cost per manual appeal in USD", ge=0
    )
    annual_claim_volume: int = Field(
        ..., description="Total claims submitted annually", ge=0
    )
    cdil_annual_cost: float = Field(
        ..., description="Annual CDIL licensing and implementation cost in USD", ge=0
    )
...
class RoiOutputs(BaseModel):
    ...
    roi_note: Optional[str] = Field(
        default=None, description="Explanatory note about ROI calculation"
    )
...
def calculate_roi(inputs: RoiInputs) -> RoiOutputs:
    ...
    documentation_denied_revenue = (
        total_denied_revenue * inputs.documentation_denial_ratio
    )

    # Step 3: Prevented denials (revenue saved by catching issues pre-submission)
    prevented_denials_revenue = (
        documentation_denied_revenue * inputs.denial_prevention_rate
    )

    # Step 4: Remaining documentation denials after prevention
    remaining_documentation_denied_revenue = (
    ...
    current_recovered_revenue = (
        remaining_documentation_denied_revenue * inputs.appeal_recovery_rate
    )
    ...
    incremental_recovery_gain = (
        remaining_documentation_denied_revenue * inputs.appeal_success_lift
    )

    # Step 7: Calculate administrative savings
    # Appeals avoided = number of denials prevented
    total_documentation_denials_count = (
        inputs.annual_claim_volume
        * inputs.denial_rate
        * inputs.documentation_denial_ratio
    )
    appeals_avoided_count = (
        total_documentation_denials_count * inputs.denial_prevention_rate
    )
    admin_savings = appeals_avoided_count * inputs.cost_per_appeal

    # Step 8: Total preserved revenue (sum of all benefits)
    total_preserved_revenue = (
